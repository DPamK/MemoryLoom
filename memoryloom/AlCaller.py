...
class AICaller:
    ...
    def chat(self, 
        ...
        if type(query) is str:
            ...
            messages = query
        if self.use_proxy:
            litellm.client_session = httpx.Client(proxies=self.proxy_url)
...
if __name__ == "__main__":
    llm = AICaller(
        model_name="openai/michat-llm",
        api_base="http://ms-13871-nstruct-lmdeploy-2-0109140455.kscn-tj5-prod2-cloudml.xiaomi.srv/v1",
        proxy="http://proxy.pt.xiaomi.com:80"
    )
    query = """
请基于这个文本描述帮我生成一个用于文生图的prompt，可以丰富文本描述，
生成的prompt需要符合如下要求：
1.请杜绝在prompt生成文本的需求
2.最后输出英文的prompt。
3.未指定风格的情况下多选择卡通风格（皮克斯动画）
4.当文本描述过于简单，请尽量丰富文本描述
5.所给文本无任何描述，也请随机生成一个prompt

只有当出现如下情况，返回“禁止”：
1.涉及政治敏感、色情、暴力等不适宜的内容
2.涉及小米公司高管形象

\n\n文本描述：{prompt}
只需要返回prompt，不需要返回其他内容。
""".format(prompt="，我的头像")
    
    response = llm.chat(query, stream=True)
